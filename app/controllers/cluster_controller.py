    ...
    async def get_clusters(crawl_task_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if crawl_task_id:
            try:
                ...
                if task_status:
                    # Task exists in memory - use task manager approach
                    ...
                    if not crawl_result:
                        raise HTTPException(status_code=404, detail=f"No crawl result found for task {crawl_task_id}")
                ...
                raise HTTPException(status_code=500, detail=f"Error finding crawl result: {str(e)}")
        else:
